...
public class MacAddress {
	...
	private byte[] macArrayRev = new byte[6]; // bits reversed (bytes not
												// reversed)
	...
	public MacAddress(byte[] arrayIn, StringBuilder builder) {
		String cr = System.getProperty("os.name").matches("(W|w)indows.*") ? "\r\n" : "\n";
		if (arrayIn.length != 6) {
			builder.append(("need six bytes array for mac address ")); // bad
																		// mac
																		// throw
																		// exception
			builder.append(cr);
	...
	public String setHexMac(String hexStr) {
		String cr = System.getProperty("os.name").matches("(W|w)indows.*") ? "\r\n" : "\n";
		...
		if (hexStr.length() != 17) {
			{
				builder.append(("bad MAC\n"));
				builder.append(cr);
			} // bad mac throw exception
			return builder.toString();
		}

		Pattern macPattern = Pattern.compile("([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}"); // matching
																					// pattern
		Matcher macMatcher = macPattern.matcher(hexStr); // pattern to be
															// matched

		if (macMatcher.find()) { // valid mac string
			String[] tokens = hexStr.split(":"); // split string into array
			for (int i = 0; i < 6; i++) {
				macArray[i] = (byte) Integer.parseInt(tokens[i], 16); // convert
																		// hex
																		// to
																		// byte
			}
	...
	/*----------------------------------------------------------------------------------------*/
	// reverses the bits in each macAddress byte and stores result in
	// macArrayRev[]
	...
	public String toHexString() {
		...
		return String.format("%02x:%02x:%02x:%02x:%02x:%02x", macArray[0], macArray[1], macArray[2], macArray[3],
	...
	public String toHexStringRev() {
		...
		return String.format("%02x:%02x:%02x:%02x:%02x:%02x", macArrayRev[0], macArrayRev[1], macArrayRev[2],
				macArrayRev[3], macArrayRev[4], macArrayRev[5]);
	}
	...
	public MacAddress clone() {
		...
		// return this;
		// return this.clone();
		return new MacAddress(macArray, new StringBuilder());
	}
	/*----------------------------------------------------------------------------------------*/
}
